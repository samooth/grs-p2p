...
import * as grsMin from "grs-minimal";
import {
  Message,
  Headers,
  Inv,
...
export default class Peer extends (EventEmitter as new () => PeerEmitter) {
  ...
  buffers: {
    msgBuffer: grsMin.utils.BufferChunksReader;
    msgBytesNeeded: number;
    blockDl:
      | {
          obj: grsMin.Block;
          buffer: grsMin.utils.BufferChunksReader;
          size: number;
          date: number;
          started: boolean;
        }
  ...
  constructor({
    ...
    this.buffers = {
      msgBuffer: new grsMin.utils.BufferChunksReader(Buffer.from([])),
      msgBytesNeeded: 0,
      blockDl: undefined,
    };
  }
  ...
  readMessage(buffer: Buffer) {
    ...
    if (command === "block" && !buffers.blockDl) {
      if (payload.length > 0) {
        const obj = new grsMin.Block({ validate });
        obj.addBufferChunk(payload);
        if (obj.txRead >= 1) {
          buffers.blockDl = {
            obj: new grsMin.Block({ validate }),
            size: sizePayload,
            buffer: new grsMin.utils.BufferChunksReader(payload),
            date: +new Date(),
            started: true,
          };
        } else {
    ...
    buffers.msgBuffer = new grsMin.utils.BufferChunksReader(remainingBuffer);
    ...
    if (command === "ping") {
      ...
      const tx = grsMin.Transaction.fromBuffer(payload);
        ...
        socket.on("data", (data: Buffer) => {
          ...
          try {
            ...
            if (buffers.blockDl) {
              ...
              let blockstream: grsMin.BlockStream | undefined;
              if (
                started ||
                this.validate ||
              ...
              if (finished) {
                ...
                buffers.msgBuffer = new grsMin.utils.BufferChunksReader(buf);
                buffers.msgBytesNeeded = 0;
  ...
  async getHeaders({
    ...
    const headers: grsMin.Header[] = await this.emitter.wait(
      "headers",
      null,
      timeoutSeconds
    );
  ...
  async broadcastTx(
    transaction: grsMin.Transaction,
  ...
  async broadcastTxs(
    transactions: grsMin.Transaction[],
